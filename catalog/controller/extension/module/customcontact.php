...
class ControllerExtensionModuleCustomContact extends Controller {
  ...
  // Add your post value to ignore in the email body content
	...
	public function index($setting) {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && !$this->request->post['contact_form'] && $this->validatefooter()) {
      ...
      //$mail->setFrom($this->request->post['email']);
			...
			$mail->setSender(html_entity_decode($this->request->post['subject2'], ENT_QUOTES, 'UTF-8'));
	...
	protected function validatefooter() {
		if(!$this->request->post['contact_form']){
			...
			if ((utf8_strlen($this->request->post['subject2']) < 3) || (utf8_strlen($this->request->post['subject2']) > 32)) {
				$this->error['subject2'] = $this->language->get('error_subject2');
			}

			if ((utf8_strlen($this->request->post['company2']) < 3) || (utf8_strlen($this->request->post['company2']) > 32)) {
				$this->error['company2'] = $this->language->get('error_company2');
			}

			if ((utf8_strlen($this->request->post['position2']) < 3) || (utf8_strlen($this->request->post['position2']) > 32)) {
				$this->error['position2'] = $this->language->get('error_position2');
			}
