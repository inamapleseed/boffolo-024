	...
	public function index($setting) {
		...
		foreach ($results as $result) {
				if (isset($this->request->get['route']) == '' || $this->request->get['route'] == 'common/home') {
					$vid_results = $this->model_catalog_news->getArticleVideos($result['news_id']);
				} else {
					$vid_results = '';
				}
				...
				if (isset($this->request->get['route']) == '' || $this->request->get['route'] == 'common/home') {
					foreach ($vid_results as $res) {
						$res['text'] = unserialize($res['text']); 
						$res['text'] = isset($res['text'][$this->config->get('config_language_id')]) ? $res['text'][$this->config->get('config_language_id')] : '' ;
						$code = '<iframe frameborder="0" allowfullscreen src="' . str_replace("watch?v=","embed/",$res['video']) . '" height="'.$res['height'].'"width="100%" ></iframe>';
