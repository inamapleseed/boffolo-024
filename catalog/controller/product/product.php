...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		$this->document->addScript('https://cdn.rawgit.com/igorlino/elevatezoom-plus/1.1.6/src/jquery.ez-plus.js');
		...
		if ($product_info) {
			...
			$download = $this->model_catalog_product->getDownload($data['download']);

			$data['download2'] = $this->model_catalog_product->getProductDownload2($product_id);
			$download2 = $this->model_catalog_product->getDownload2($data['download2']);

			$data['download3'] = $this->model_catalog_product->getProductDownload3($product_id);
			$download3 = $this->model_catalog_product->getDownload3($data['download3']);

			$data['download4'] = $this->model_catalog_product->getProductDownload4($product_id);
			$download4 = $this->model_catalog_product->getDownload4($data['download4']);

			$data['download5'] = $this->model_catalog_product->getProductDownload5($product_id);
			$download5 = $this->model_catalog_product->getDownload5($data['download5']);

			if($data['download']){
				$data['download'] = $this->url->link('product/product/download', 'document_id='.$data['download']);
				$data['download_mask'] = $download['mask'];
			}

			if($data['download2']){
				$data['download2'] = $this->url->link('product/product/download2', 'document_id='.$data['download2']);
				$data['download2_mask'] = $download2['mask2'];
			}

			if($data['download3']){
				$data['download3'] = $this->url->link('product/product/download3', 'document_id='.$data['download3']);
				$data['download3_mask'] = $download3['mask3'];
			}

			if($data['download4']){
				$data['download4'] = $this->url->link('product/product/download4', 'document_id='.$data['download4']);
				$data['download4_mask'] = $download4['mask4'];
			}
			if($data['download5']){
				$data['download5'] = $this->url->link('product/product/download5', 'document_id='.$data['download5']);
				$data['download5_mask'] = $download5['mask5'];
			}
	...
	public function download(){
		...
		if ($download_info) {
			...
			// $fname = 'image/catalog/'.$download_info['filename'];
	...
	public function download2(){
		...
		if( isset($this->request->get['document_id']) && (int)$this->request->get['document_id'] ){
			$product_pdf_id2 = (int)$this->request->get['document_id'];
		}

		$download_info = $this->model_catalog_product->getDownload2($product_pdf_id2);

		if ($download_info) {
			$file = DIR_DOWNLOAD . $download_info['filename2'];
			$mask = basename($download_info['mask2']);
	...
	public function download3(){
		...
		if( isset($this->request->get['document_id']) && (int)$this->request->get['document_id'] ){
			$product_pdf_id3 = (int)$this->request->get['document_id'];
		}

		$download_info = $this->model_catalog_product->getDownload3($product_pdf_id3);
		if ($download_info) {
			$file = DIR_DOWNLOAD . $download_info['filename3'];
			$mask = basename($download_info['mask3']);
	...
	public function download4(){
		...
		if( isset($this->request->get['document_id']) && (int)$this->request->get['document_id'] ){
			$product_pdf_id4 = (int)$this->request->get['document_id'];
		}

		$download_info = $this->model_catalog_product->getDownload4($product_pdf_id4);

		if ($download_info) {
			$file = DIR_DOWNLOAD . $download_info['filename4'];
			$mask = basename($download_info['mask4']);
	...
	public function download5(){
		...
		if( isset($this->request->get['document_id']) && (int)$this->request->get['document_id'] ){
			$product_pdf_id5 = (int)$this->request->get['document_id'];
		}

		$download_info = $this->model_catalog_product->getDownload5($product_pdf_id5);

		if ($download_info) {
			$file = DIR_DOWNLOAD . $download_info['filename5'];
			$mask = basename($download_info['mask5']);
