<?php
	class ControllerCommonHeader extends Controller {
		public function index() {
			...
			$data['announcement'] = $this->config->get('config_announcement');
			
			// For page specific css
			
			$data['class'] = 'common-home';
			
			$data['announcement'] = $this->load->controller('extension/module/announcement/migrate');
			...
			$menu4 = $this->load->controller('common/menu', 4);

			$this->fill_sub_categories($menu);

			$data['menu'] = $this->craftHtml($menu);
			$data['menu4'] = $this->craftHtml($menu4);
		...
		private function craftHtml($menu = array(), $level = 0, $menu_part = 0 ){
			...
			foreach($menu as $i => $link){
				...
				if($sub_menu) {
					...
					if($level){
						...
						$menus .= '	<a href="'.$href.'" '.$tab_option.' alt="' . $name . '" ><img src="./image/'.$nav_img.'" alt="" class="img-responsive">' . $inner_text . '</a>' . $sub_menu;
					}
				} 
				else{ 
					$menus .= '<li class="'.($link['active']?'active':'').'">';
					$menus .= '	<a href="'.$href.'" '.$tab_option.' alt="' . $name . '" ><img src="./image/'.$nav_img.'" alt="" class="img-responsive">'  . $inner_text . '</a>';
				}
			...
			if( trim($menus) != '' ){
				if($level){
					$menus = '<ul class="content">' . $menus . '</ul>';
				}
				else{
					$menus = 
