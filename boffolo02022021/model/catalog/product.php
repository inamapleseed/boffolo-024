...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['mask2']) && text($data['mask2']) != '') {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf2 SET product_id='". (int)$product_id ."', filename2 = '" . $this->db->escape($data['filename2']) . "', mask2 = '" . $this->db->escape($data['mask2']) . "'");
		}
		if (isset($data['mask3']) && text($data['mask3']) != '') {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf3 SET product_id='". (int)$product_id ."', filename3 = '" . $this->db->escape($data['filename3']) . "', mask3 = '" . $this->db->escape($data['mask3']) . "'");
		}
		if (isset($data['mask4']) && text($data['mask4']) != '') {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf4 SET product_id='". (int)$product_id ."', filename4 = '" . $this->db->escape($data['filename4']) . "', mask4 = '" . $this->db->escape($data['mask4']) . "'");
		}
		if (isset($data['mask5']) && text($data['mask5']) != '') {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf5 SET product_id='". (int)$product_id ."', filename5 = '" . $this->db->escape($data['filename5']) . "', mask5 = '" . $this->db->escape($data['mask5']) . "'");
		}
	...
	public function editProduct($product_id, $data) {
		...
		//end of 1
		if (isset($data['product_pdf_id2']) && $data['product_pdf_id2']) {
			if ($data['mask2']) {
				// Update
				$this->db->query("UPDATE `". DB_PREFIX ."product_pdf2` SET mask2='" . $this->db->escape($data['mask2']) . "', filename2='" . $this->db->escape($data['filename2']) . "' WHERE product_pdf_id2='" . (int)$data['product_pdf_id2'] . "'");
			}
			else{
				// Delete
				$this->db->query("DELETE FROM `". DB_PREFIX ."product_pdf2` WHERE product_pdf_id2='" . (int)$data['product_pdf_id2'] . "'");
			}
		}
		elseif ($data['mask2'] ) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf2 SET product_id='". (int)$product_id ."', filename2 = '" . $this->db->escape($data['filename2']) . "', mask2 = '" . $this->db->escape($data['mask2']) . "'");
		...
		if (isset($data['product_pdf_id3']) && $data['product_pdf_id3']) {
			if ($data['mask3']) {
				// Update
				$this->db->query("UPDATE `". DB_PREFIX ."product_pdf3` SET mask3='" . $this->db->escape($data['mask3']) . "', filename3='" . $this->db->escape($data['filename3']) . "' WHERE product_pdf_id3='" . (int)$data['product_pdf_id3'] . "'");
			}
			else{
				// Delete
				$this->db->query("DELETE FROM `". DB_PREFIX ."product_pdf3` WHERE product_pdf_id3='" . (int)$data['product_pdf_id3'] . "'");
			}
		}
		elseif ($data['mask3'] ) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf3 SET product_id='". (int)$product_id ."', filename3 = '" . $this->db->escape($data['filename3']) . "', mask3 = '" . $this->db->escape($data['mask3']) . "'");
		...
		//end of 3
		if (isset($data['product_pdf_id4']) && $data['product_pdf_id4']) {
			if ($data['mask4']) {
				// Update
				$this->db->query("UPDATE `". DB_PREFIX ."product_pdf4` SET mask4='" . $this->db->escape($data['mask4']) . "', filename4='" . $this->db->escape($data['filename4']) . "' WHERE product_pdf_id4='" . (int)$data['product_pdf_id4'] . "'");
			}
			else{
				// Delete
				$this->db->query("DELETE FROM `". DB_PREFIX ."product_pdf4` WHERE product_pdf_id4='" . (int)$data['product_pdf_id4'] . "'");
			}
		}
		elseif ($data['mask4'] ) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf4 SET product_id='". (int)$product_id ."', filename4 = '" . $this->db->escape($data['filename4']) . "', mask4 = '" . $this->db->escape($data['mask4']) . "'");
		...
		if (isset($data['product_pdf_id5']) && $data['product_pdf_id5']) {
			if ($data['mask5']) {
				// Update
				$this->db->query("UPDATE `". DB_PREFIX ."product_pdf5` SET mask5='" . $this->db->escape($data['mask5']) . "', filename5='" . $this->db->escape($data['filename5']) . "' WHERE product_pdf_id5='" . (int)$data['product_pdf_id5'] . "'");
			}
			else{
				// Delete
				$this->db->query("DELETE FROM `". DB_PREFIX ."product_pdf5` WHERE product_pdf_id5='" . (int)$data['product_pdf_id5'] . "'");
			}
		}
		elseif ($data['mask5'] ) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_pdf5 SET product_id='". (int)$product_id ."', filename5 = '" . $this->db->escape($data['filename5']) . "', mask5 = '" . $this->db->escape($data['mask5']) . "'");
	...
	public function deleteProduct($product_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_pdf2 WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_pdf3 WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_pdf4 WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_pdf5 WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_description WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_discount WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT p.*, pd.name, product_pdf_id, filename, mask, product_pdf_id2, filename2, mask2, product_pdf_id3, filename3, mask3, product_pdf_id4, filename4, mask4, product_pdf_id5, filename5, mask5, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS keyword FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN `".DB_PREFIX."product_pdf` pp ON (pp.product_id = p.product_id)  LEFT JOIN `". DB_PREFIX ."product_pdf2` pp2 ON (pp2.product_id = p.product_id) LEFT JOIN `". DB_PREFIX ."product_pdf3` pp3 ON (pp3.product_id = p.product_id) LEFT JOIN `". DB_PREFIX ."product_pdf4` pp4 ON (pp4.product_id = p.product_id) LEFT JOIN `". DB_PREFIX ."product_pdf5` pp5 ON (pp5.product_id = p.product_id)  WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
