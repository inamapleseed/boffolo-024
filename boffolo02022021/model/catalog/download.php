...
class ModelCatalogDownload extends Model {
	public function addDownload($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "download SET filename = '" . $this->db->escape($data['filename']) . "', description = '" . $this->db->escape($data['description']) . "', description2 = '" . $this->db->escape($data['description2']) . "', moreinfo = '" . $this->db->escape($data['moreinfo']) . "', category = '" . $this->db->escape($data['category']) . "', mask = '" . $this->db->escape($data['mask']) . "', date_added = NOW()");
	...
	public function editDownload($download_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "download SET filename = '" . $this->db->escape($data['filename']) . "', description = '" . $this->db->escape($data['description']) . "', description2 = '" . $this->db->escape($data['description2']) . "', category = '" . $this->db->escape($data['category']) . "', moreinfo = '" . $this->db->escape($data['moreinfo']) . "', mask = '" . $this->db->escape($data['mask']) . "' WHERE download_id = '" . (int)$download_id . "'");
